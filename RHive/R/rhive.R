...
rhive.init <- function(hive=NULL,libs=NULL, hadoop_home=NULL, hadoop_conf=NULL, hlibs=NULL, verbose=FALSE){
  ...
  if (hadoop_home=="") {
    print("HADOOP_HOME is missing. HDFS functions won't work")
...
.checkConnection <- function(hiveclient=rhive.defaults('hiveclient')) {
  if (is.null(hiveclient)) {
    stop("There is no connection to a hiveserver. try to command 'rhive.connect(hive-server-ip)'")
  }
}
...
# --------------------------------------------------------------
# Functions which have different naming rule (Alias functions)
# --------------------------------------------------------------
